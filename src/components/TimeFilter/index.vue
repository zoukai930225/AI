<template>
  <div class="time-filter">
    ...
    <div v-if="isShowTime && staticsTimeInfo" class="time-filter-staticsTime">({{ `${showStr}:${staticsTimeInfo}` }})
    </div>
    <div class="topDateFilter-bg">
      <div class="topDateFilter-bg-timeSelect" v-for="(value, index) in timeOptions" :key="index"
        :class="{ 'topDateFilter-bg-timeSelect-selected': selectValue.index === index }"
        @click="timeSelectAction(value)">
        <div class="topDateFilter-bg-timeSelect-name">{{ value.label }}
      ...
      <el-tooltip v-if="isTooltip" :disabled="!tooltipInfo" effect="dark" :content="tooltipInfo" placement="bottom">
        <el-icon :class="[!!tooltipInfo ? 'topDateFilter-bg-tips' : 'topDateFilter-bg-tips-dis']" color="#606266">
          <Warning />
        </el-icon>
      </el-tooltip>
      <template v-if="isArrow && showArrow">
        <el-icon :color="color" :style="{ marginRight: '6px', marginLeft: '10px', cursor: isCursor }"
          @click="arrow(-1)">
          <ArrowLeft />
        </el-icon>
